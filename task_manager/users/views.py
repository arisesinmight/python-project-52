...
class UserUpdateView(
    LoginRequired, CheckUserMixin, SuccessMessageMixin, UpdateView
):
    model = User
    ...
    permission_denied_message = _(
        "You don't have access for editing other users profile."
    )
...
class UserDeleteView(LoginRequired, CheckUserMixin, DeleteView):
    ...
    permission_denied_message = _(
        "You don't have access for deleting other users profile."
    )
    permission_denied_url = 'users_index'
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.object.got_tasks.exists() or self.object.created_tasks.exists():
            messages.warning(self.request, _(
                "Unable to delete user that's in work."
            ))
